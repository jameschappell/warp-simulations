"""
This script was downloaded from the Warp PIC simulation code example scripts.
Find at https://bitbucket.org/berkeleylab/warp
...
def beam_dens_func(x, y, z):
    ...
    n = n * (z - beam_zmin) * (beam_zmax - z) * 4 / (
                                                    beam_zmax - beam_zmin) ** 2
...
plasma_injector = PlasmaInjector(elec, ions, w3d, top, dim,
                                 ...
                                 plasma_zmax, plasma_xmax, plasma_ymax,
                                 plasma_dens_func,
                                 uz_m=plasma_uz_m)
...
if (write_lab_frame == 1) and (gamma_boost > 1):
    diag0 = BoostedFieldDiagnostic(zmin_lab, zmax_lab, clight,
                                   dt_snapshot_lab, Ntot_snapshot_lab,
                                   gamma_boost,
    ...
    diag3 = BoostedParticleDiagnostic(zmin_lab, zmax_lab, clight,
                                      dt_snapshot_lab, Ntot_snapshot_lab,
                                      gamma_boost,
                                      period=diag_period, top=top, w3d=w3d,
                                      em=em, comm_world=comm_world,
                                      species={species.name: species for
                                               species in listofallspecies})
